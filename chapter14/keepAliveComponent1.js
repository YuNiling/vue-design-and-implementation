...
function onDeactivated(fn) {
  if (currentInstance) {
    currentInstance.deActivated.push(fn);
  } else {
...
function onActivated(fn) {
  if (currentInstance) {
    currentInstance.activated.push(fn);
  } else {
    ...
    if (typeof type === 'string') {
      ...
      if (!n1) {
        ...
        if (n2.keptAlive) {
          ...
          n2.component.activated && n2.component.activated.forEach(hook => hook.call(n2.component.renderContext));
        } else {
  ...
  function mountComponent(vnode, container, anchor) {
    ...
    effect(() => {
      ...
      if (!instance.isMounted) {
        ...
        // 在这里调用 mounted 钩子，遍历 instance.mounted 数组并逐个执行
        instance.mounted && instance.mounted.forEach(hook => hook.call(renderContext));
        // 首次挂载，激活 onActivated
        if (vnode.shouldKeepAlive) {
          instance.activated && instance.activated.forEach(hook => hook.call(renderContext));
        }
      } else {
  ...
  function unmount(vnode) {
    ...
    if (vnode.type === Fragment) {
      ...
      if (vnode.shouldKeepAlive) {
        ...
        instance.beforeUnmount && instance.beforeUnmount.forEach(hook => hook.call(instance.renderContext));
        // 对于需要被 KeepAlive 的组件，我们不应该真的卸载它，而应该调用该组件的父组件，
        // 即 KeepAlive 组件的 _deActivate 函数使其失活
        instance._deActivate(vnode);
        vnode.component.deActivated && vnode.component.deActivated.forEach(hook => hook.call(vnode.component.renderContext));
        instance.unmounted && instance.unmounted.forEach(hook => hook.call(instance.renderContext));
      } else {
        const instance = vnode.component.instance;
        instance.beforeMount && instance.beforeUnmount.forEach(hook => hook.call(instance.renderContext));
...
const Comp1 = {
  ...
  setup() {
    onActivated(() => {
      console.log('Comp1 激活');
    });
    onDeactivated(() => {
      console.log('Comp1 失活');
    });
    return {};
  },
  render() {
      ...
      children: [
        {
          key: 'Comp1_1',
          type: Text,
          children: `Count1 的值是：${this.count1}`
        },
        {
          key: 'Comp1_2',
          type: 'button',
...
const Comp2 = {
  ...
  setup() {
    onActivated(() => {
      console.log('Comp2 激活');
    });
    onDeactivated(() => {
      console.log('Comp2 失活');
    });
    return {};
  },
  render() {
      ...
      children: [
        {
          key: 'Comp2_1',
          type: Text,
          children: `Count2 的值是：${this.count2}`
        },
        {
          key: 'Comp2_2',
          type: 'button',
...
const Comp3 = {
  ...
  setup() {
    onActivated(() => {
      console.log('Comp3 激活');
    });
    onDeactivated(() => {
      console.log('Comp3 失活');
    });
    return {};
  },
  render() {
      ...
      children: [
        {
          key: 'Comp3_1',
          type: Text,
          children: `Count3 的值是：${this.count3}`
        },
        {
          key: 'Comp3_2',
          type: 'button',
