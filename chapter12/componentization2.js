// * 组件实例与组件的生命周期、props 与组件的被动更新
// * 1.组件实例本质上是一个状态集合（或一个对象），它维护着组件运行过程中的所有信息，例如注册到主角的生命周期函数、组件渲染的子树（subTree）、组件是否已经被挂载、组件自身的状态（data）等等。
// * 2.由父组件自更新所引起的子组件更新叫做子组件的被动更新。

import { reactive, effect, flushQueue, shallowReactive } from '../reactive.js';
...
// 测试：<MyComponent title="A Big Title" />
const MyComponent = {
  name: 'MyComponent',
