...
// * 数组就是一种异质对象，它除了 [[DefineOwnProperty]] 这个内部方法之外，其他内部方法的逻辑都与常规对象想通过。
// * for...of 用来遍历可迭代对象。
// * 一个对象能否被迭代，取决于该对象或者该对象的原型是否实现了 @@iterator 方法。@@iterator 指的就是 Symbol.iterator 的值。
...
// 测试1：通过索引读取或者设置数组元素的值时，代理对象的 get/set 拦截函数是否执行
...
// 测试2：通过索引设置元素值时，可能会隐式地修改 length 的属性值。因此在触发响应时，也应该触发与 length 属性相关联的副作用函数重新执行。
// const arr = reactive(['foo']);
// effect(() => {
//   console.log(arr.length);
// });
// // arr[0] = 'bar'; // 设置索引 0 的值，数组的长度不变，不触发响应
// arr[1] = 'bar'; // 设置索引 1 的值，会导致数组的长度变为 2

// 测试3：修改数组的 length 属性会隐式地影响数组元素。当修改 length 属性值时，只有那些索引值大于或等于新的 length 属性值的元素才需要触发响应。
// const arr = reactive(['foo']);
// effect(() => {
//   console.log(arr[0]);
// });
// // arr.length = 100; // 将数组的长度修改为 100，对第 0个元素没有影响，不触发响应
// arr.length = 0; // 将数组的长度修改为 0，导致第 0 个元素被删除，触发响应

// 测试4：无论是为数组添加新元素，还是直接修改数组的长度，本质上都是修改了数组的 length 属性。一旦数组的 length 属性被修改，那么 for...in 循环对数组的遍历结果就会改变，这时就会触发响应。
// const arr = reactive(['foo']);
// effect(() => {
...
//   }
// });
// // arr[1] = 'bar'; // 添加新的元素，触发响应
// arr.length = 2; // 修改数组长度，触发响应
...
// 测试6：数组内建了 Symbol.iterator 方法的实现
// const arr = [1, 2, 3, 4, 5];
// const itr = arr[Symbol.iterator](); // 获取并调用数组内建的迭代器方法
...
// 测试9：迭代数组时，只需要在副作用函数与数组的长度与索引之间建立联系，就能实现响应式的 for...of 迭代
...
//   }
// });
// arr[1] = 'bar'; // 修改索引 1 的值为 bar，触发响应
// arr.length = 1; // 修改数组长度，触发响应

// 测试10：数组的 values 方法的返回值实际上就是数组内建的迭代器
...
// 测试11：数组的内部方法其实都依赖了对象的基本语义。includes 是为了找到给定的值，它内部会访问数组的 length 属性以及数组的索引，因此修改某个索引执行的元素值后会触发响应。
...
// 测试12：arr[0] 得到的是一个代理对象，而在 includes 方法内部也会通过 arr 访问数组元素，从而得到一个代理对象，通过 reativeMap 缓存使得两个代理对象相同。
// const obj = {};
// const arr = reactive([obj]);
// console.log(arr.includes(arr[0]));

// 测试13：includes 内部的 this 指向的是代理对象 arr，并且在获取数组元素时得到的值也是代理对象，拿原始对象 obj 去查找不能找到，这时就需要重写 includes 方法，类似的处理方法有：indexOf、lastIndexOf。
// const obj = {};
// const arr = reactive([obj]);
...
// 测试14：push 方法的调用会间接读取、设置 length 属性，fn1 会导致 fn2 调用，fn2 会导致 fn1 调用，循环往复，最终导致栈溢出。
// 解决方案：“屏蔽”对 length 属性读取，从而避免在它与副作用函数之间建立响应性。
// 因为数组的 push 方法在语义上是修改操作，而非读取操作，所以避免建立响应性联系并不会产生其他副作用，类似的处理方法有：pop、shift、unshift、splice。
const arr = reactive([]);
// 第一个副作用函数
effect(function fn1() {
  arr.push(1);
