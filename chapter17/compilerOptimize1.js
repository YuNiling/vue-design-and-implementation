// ** 动态节点收集与补丁标志
// 1. 传统 Diff 算法无法利用编译时提取到的任何关键信息，这导致渲染器中运行时不可能去做相关优化。
// 2. patchFlag（补丁标志）的值是一个数字，只要虚拟节点存在该属性，这个节点就是动态节点。
// 3. Block 是一个虚拟 DOM 节点，带有 dynamicChildren 属性的虚拟节点成为“块”，即 Block。
// 4. 一个 Block 不仅能够收集它的直接动态子节点，还能收集所有动态“子代”节点。
// 5. 一个普通的虚拟节点变成 Block 节点：根节点，任何带有 v-for、v-if/v-else-if/v-else 等指令的节点。
// 6. 由于 createVNode 函数的调用是层层嵌套结构，执行顺序是“内层先执行，外层后执行”，所以执行内层时用栈收集动态节点，在执行外层时从栈弹出节点。
// 7. 动态节点集合：渲染器在执行更新时跳过所有静态节点；单个动态节点：针对性的完成靶向更新。
...
function render() {
  ...
  return (openBlock(), createBlock('div', { id: 'foo' }, [
    createVNode('p', { class: 'foo' }, foo, PatchFlags.TEXT),
