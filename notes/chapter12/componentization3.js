// * setup 函数的作用与实现
// * 1.在组件的整个生命周期中，setup 函数只会被挂载一次，它的返回值可以有两种情况。
...
// 测试1：setup 函数返回一个函数，该函数将作为组件的 render 函数
// const MyComponent = {
//   setup() {
//     return () => {
...
// 测试2：setup 函数返回一个对象，该对象中包含的数据将暴露给模板使用
const MyComponent = {
  setup() {
    const count = ref(9);
    return {
  ...
  render() {
    return { type: 'h1', children: `count is: ${this.count}` }
  }
};
const vnode = {
  type: MyComponent
