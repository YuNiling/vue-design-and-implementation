// ** Block 树
// 1. dynamicChildren 数组中收集的动态节点上忽略虚拟 DOM 树层级的。
// 2. 传统 Diff 算法的一个非常重要的前置条件：进行 Diff 操作的节点必须时同层级节点。
// 3. 结构化指令（v-if/v-for）回导致更新前后模板的结构发生变化，即模板不稳定。
// 4. 结构不稳定：从结果上看，指的是更新前后一个 block 的 dynamicChildren 数组中收集的动态节点的数量或顺序不一致。
// 5. 稳定的 Fragment 情况：v-for 指令表达式是常量，模板中有多个根节点。
...
import { Text, Comment, Fragment, KeepAlive, Teleport, Transition } from '../NODE_TYPE.js';
...
function render1(ctx = {}) {
  return (openBlock(), createBlock('div', null, [
    ctx.foo 
      ? createBlock('section', { key: 0 /* key 值回根据不同的 Block 而发生变化 */ }, [
          createVNode('p', null, ctx.a, PatchFlags.TEXT)
        ])
      : createBlock('section', { key: 1 }, [
...
const vnode1 = render1({
  foo: true,
  a: 'test1'
});
console.log('vnode1', vnode1);
const vnode2 = render1({
  foo: false,
  a: 'test2'
});
...
const vnode3 = render2({
  list: [1, 2]
});
console.log('vnode3', vnode3);
const vnode4 = render2({
  list: [3]
});
