// ** v-once
// 1. 被 v-once 包裹的动态节点在组件更新时，不会参与 Diff 操作。
// 2. 避免组件更新时重新创建虚拟 DOM 带来的性能开销。
// 3. 避免无用的 Diff 开销。
...
let shouldTrack = true; // 全局变量，表示是否进行依赖收集
...
function createBlock(tag, props, children) {
  ...
  block.dynamicChildren = shouldTrack ? currentDynamicChildren : null;
...
/**
 * 设置这段 VNode 是否被 Block 收集
 * @param {number} value 大于0，启用依赖收集，否则，禁止依赖收集
 */
...
function render(ctx, cache) {
  return (openBlock(), createBlock('div', null, [
    cache[1] || (
      ...
      cache[1] = createVNode('p', null, ctx.foo, PatchFlags.TEXT),
...
const vnode1 = render({ foo: 'foo' }, cache);
console.log(vnode1);
renderer.render(vnode1, document.querySelector('#app'));

const vnode2 = render({ foo: 'yrd' }, cache);
console.log(vnode2);
