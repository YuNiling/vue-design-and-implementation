// * 函数式组件
// * 1.一个函数式组件本质上就是一个普通函数，该函数的返回值时虚拟 DOM。
// * 2.在 Vue.js 3 中使用函数式组件，主要是因为它的简单性，而不是因为它的性能好。
// * 3.函数式组件没有自身状态，但它仍然可以接收由外部传入的props。为了给函数式组件定义 props，我们需要在组件函数上添加静态的 props 属性。
...
const vnode = {
  type: MyFuncComp,
  props: {
    title: 'props 测试内容，请点击测试 emit',
    class: 'custom-class',
    style: { color: 'red' },
    onCustomClick: (msg) => {
      console.log('customClick', msg);
    }
  },
  children: {
    header() {
