...
// 测试：基础、超时与 Error 组件、延迟与 Loading 组件、重试机制
const InnerComp = {
  setup() {
    onUnmounted(() => {
      console.log('InnerComp--onUnmount');
    });
    return () => {
      return {
        type: 'div', 
...
const ErrorComp = {
  ...
  setup() {
    onUnmounted(() => {
      console.log('ErrorComp--onUnmount');
    });
    return {};
  },
  render() {
...
const LoadingComp = {
  setup() {
    onBeforeUnmount(() => {
      console.log('LoadingComp--onBeforeUnmount');
    });
    onUnmounted(() => {
      console.log('LoadingComp--onUnmount');
    });
    return {};
  },
  render() {
...
const Max_Retries = 2;
let count = 0;
const loaderComp = () => new Promise((resolve, reject) => {
  setTimeout(() => {
...
const AsyncComp = defineAsyncComponent({
  ...
  delay: 200, // 延迟 200 ms 展示 Loading 组件
...
const OuterComp = {
  setup() {
    onUnmounted(() => {
      console.log('OuterComp--onUnmount');
    });
